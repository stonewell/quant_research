...
## 9. Shared DuckDB OHLCV cache (`cache.duckdb`)
...
directory holding the shared DuckDB cache database `data/cache.duckdb`.

`common.duckdb_cache.DuckDBCache` provides the centralized storage engine:
- **Schema**:
  - `ohlcv_bars`: keyed by `(provider, symbol, date)`. Stores base daily resolution OHLCV data. Multi-interval queries (`1d`, `1wk`, `1mo`, etc.) and date slices are dynamically resampled on the fly.
  - `asset_sync_metadata`: keyed by `(provider, symbol)`. Tracks `earliest_available_date`, `latest_available_date`, and `is_earliest_known` (flagged when a query requests dates prior to asset inception/listing) so upstream providers are never re-queried for unavailable historical ranges.
- **Cache Invalidation & Coverage**:
  - Cache never expires (no TTL or age-based expiration).
  - Upstream data providers are queried only when the requested date range is not already covered by the cached data boundaries.
- **Synthetic Data**:
  - Synthetic data (`SyntheticDataProvider` / `'synthetic'`) always completely skips the DuckDB cache (zero reads, zero writes).
- **Migration**:
  - Legacy CSV cache files can be imported via `scripts/migrate_cache_csv_to_duckdb.py`, which imports valid historical CSVs while skipping synthetic data files.
