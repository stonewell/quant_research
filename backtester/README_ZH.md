...
| `--china-trading` | 标志，默认关闭 | 应用中国 A 股交易限制规则：涨跌停限制（主板 10%、创业板/科创板 20%、北交所 30%），涨停禁止买入、跌停禁止卖出，严格 T+1 结算规则，100 股一手整数倍委托，卖方单边 0.05% 印花税 |
| `--us-trading` | 标志，默认关闭 | 应用美股交易规则：1 股最小委托单位，卖方单边 0.00278% SEC 规费，T+0 融资日内回转交易 |
| `--hk-trading` | 标志，默认关闭 | 应用港股交易规则：每手整手买卖单位（默认 100 股或股票特定手数如汇丰 400 股），双边 0.1085% 印花税及交收费/证监会征费（买卖双向征收），T+0 日内回转交易 |
